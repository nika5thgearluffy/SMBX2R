...
function horikawaTools.loadLevel(filename, loadingEpisodeName, warpIdx)
	...
	for indexer = 1, EP_LIST_COUNT do
		episodeName = tostring(mem(EP_LIST_PTR + (indexer - 1) * 0x18 + 0x0, FIELD_STRING))
		if episodeName == loadingEpisodeName then
...
function horikawaTools.printCenterX(toPrint, y)
	Text.print(toPrint, (800 - (#toPrint) * 18) * 0.5, y)
end
...
function horikawaTools.findRoot()
	...
	while (table.getn(listOfFiles) == 0) do
		directoryString = ".\\" .. directoryLevelsUp .. "scripts"
