-- Disable npcparse when this is loaded
local npcParse = require('npcparse')
npcParse.active = false
...
do	
	-- Resource path
	cinematX.resourcePath = "..\\..\\..\\scripts\\Legacy\\cinematX\\"
...
do 
	...
	function Actor:update ()
		...
		-- Prevent despawning  --NPCMemSet,NPC ID,0x12A,55,0,0,w
...
do
		...
		if   (cinematX.showDebugInfo == true)   then
			...
			if  (cinematX.showConsole == true)  then	
				...
				if  (cheatStr ~= nil)  then
					printText ("INPUT: " .. Misc.cheatBuffer(), 4, 20, 580)
				end
			...
			--]]
				...
				--]]
				--printText ("_"..v.msg.."_", 4, myX, myY-96)
	...
	function cinematX.displayDebug_indexedKeys ()
		...
		i = 0
		for k,v in pairs (cinematX.indexedActors) do
				printText ("NO KEY "..tostring(k)..", "..v.smbxObjRef.msg, 4, 400, 120+20*i)
			i = i+1
		end
	...
	function cinematX.processCheat (cheatString)
		local tempStr = Misc.cheatBuffer()
		local isTrue = false
...
do
	...
	function cinematX.processNPCMessage (npcIndex)
		...
		if  	(thisActor.sceneString ~= nil)   then
			...
			tempFunctB = Misc.getCustomEnvironment()[tempFunctStr]
