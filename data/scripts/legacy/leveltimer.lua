...
local resPath = getSMBXPath() .. "\\scripts\\Legacy\\leveltimer"; --res path
...
lf_levelTimerApi.willKillPlayer = true;
lf_levelTimerApi.outtatime = false;
...
function lf_levelTimerApi.onLoopOverride()
  ...
  --Text.print("Seconds left..." .. tostring(secondsleft - timeelapsed), 0, 15);
    ...
    if(timeelapsed >= secondsleft) then
      passTime = false;
	  lf_levelTimerApi.outtatime = true;
      if(beatLevel == false) then
		if (lf_levelTimerApi.willKillPlayer == true) then
...
function lf_levelTimerApi.getTimerState()
