...
local function AutoscrollCamera()
	...
	if not (player.forcedState == 3 or player.forcedState == 7) then
...
function SMWCam.CameraLogic()
	...
	if warpingOut then
		if (player.forcedState == 3 or player.forcedState == 7) and camlockState ~= AUTOSCROLLING then warpingOut = true else warpingOut = false end
	end
