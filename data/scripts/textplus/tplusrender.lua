...
local function queueSeg(x, y, seg, startIdx, endIdx, out, cursor, limit, crisp)
	...
	local italic = (seg.fmt.italic or 0)
	...
	for idx = startIdx, endIdx do
		...
		if glyph then
			...
			do
				...
				local topoffset = 0
				if italic ~= 0 then
					topoffset = italic*xscale
				end
				
				local i = #vCoords
				vCoords[i+1], vCoords[i+2] = x1, y2
				vCoords[i+3], vCoords[i+4] = x1 + topoffset, y1
				vCoords[i+5], vCoords[i+6] = x2 + topoffset, y1
				vCoords[i+7], vCoords[i+8] = x2 + topoffset, y1
				vCoords[i+9], vCoords[i+10] = x2, y2
				vCoords[i+11], vCoords[i+12] = x1, y2
