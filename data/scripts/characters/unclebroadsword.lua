...
unclebroadsword.smallLimitsMoves = true
...
local function UpdateInputs(p)
	...
	if  not is_hurt and not inforcedanim()  then
		...
		if  p.keys.run == KEYS_PRESSED  and  not (is_hurt or statued())  then
			...
			elseif	unclebroadsword.attackState == ATKSTATE.SWIPE1  and  attack_timer <= DURATION[ATKSTATE.SWIPE1]*3/2  and  cancelComboCheck  then
			...
			elseif	unclebroadsword.attackState == ATKSTATE.SWIPE2  and  attack_timer <= DURATION[ATKSTATE.SWIPE2]*3/2  and  not ducking()  and  smallLimitCheck  and  cancelComboCheck  then
	...
	if  player.character == CHARACTER_UNCLEBROADSWORD and  not is_hurt  and  not inforcedanim()  and  not unclebroadsword.extraConfig  then
		...
		if keycode == KEY_X and not (is_hurt or statued()) then
			...
			elseif	unclebroadsword.attackState == ATKSTATE.PAUSE1	then
				...
				attack_timer = DURATION[ATKSTATE.SWIPE2]*3
				Audio.SfxPlayCh(-1, Audio.SfxOpen(pm.getSound(CHARACTER_UNCLEBROADSWORD, sfx.swipe)), 0)
			elseif	unclebroadsword.attackState == ATKSTATE.SWIPE2 and attack_timer <= DURATION[ATKSTATE.SWIPE2]*3/2 and not ducking() and smallLimitCheck then
				queue_state = ATKSTATE.LUNGE_COMBO			-- Queue up combo lunge if during second swipe
...
local function UpdateAttackState() ---------------------------------------------------------------- Advance attack state timers	
	...
	if attack_timer <= 0 then
		...
		-- Check if charging
		if  smallLimitCheck  and  standing()  then
	...
	-- Reset the stall & fall canceled flag
	if  grounded()  or  submerged()  or  sliding()  or  climbing()  or  (statued() and mounted())  then
		canceled_stallnfall = false
	end

	--Text.print("Double jump ready: "..tostring(doublejump_ready), 3, 20,80)
	--Text.print("PYGP flag: "..tostring(player:mem(0x60, FIELD_BOOL)), 3, 20,100)
	--Text.print("Downstall timer: "..tostring(holddown_timer), 3, 20,120)
	--Text.print("Alt code: "..tostring(unclebroadsword.extraConfig), 3, 20,140)
end
...
function unclebroadsword.onTick()
	if  player.character == CHARACTER_UNCLEBROADSWORD  and  player:mem(0x13E,FIELD_WORD) == 0  then
		...
		if  unclebroadsword.extraConfig  then
			UpdateInputs(player)
		end
