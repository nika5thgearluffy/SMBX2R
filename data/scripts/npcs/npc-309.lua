...
local function overlapsWithBlock(v)
	for k,b in Block.iterateIntersecting(v.x, v.y + v.height, v.x + v.width, v.y + v.height) do
...
local function checkAwake(v, dist, data, needsWhistle)
	...
	if ((not needsWhistle) and math.abs(dist) < 300) or whistle.getActive() then
		data.state = ST_TELE
		for k,b in Block.iterateIntersecting(v.x + 2, v.y + 2, v.x + v.width - 2, v.y + v.height - 2) do
			if isValidBlock(b) and Block.SOLID_MAP[b.id] then
				return false
			end
		end
		for k,b in Block.iterateIntersecting(v.x + 2, v.y + v.height + 2, v.x + v.width - 2, v.y + v.height + 4) do
