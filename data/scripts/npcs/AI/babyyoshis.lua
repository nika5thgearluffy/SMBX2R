...
function babyYoshis.onTickEndNPC(v)
	...
	if bypass or v.ai2 < cfg.hunger then
		...
		local collidingNPCs = colliders.getColliding{
			...
			filter = function(other)
				if not NPC.HITTABLE_MAP[other.id] and not NPC.POWERUP_MAP[other.id] and not berries.idMap[other.id] then
					return false
				end
		...
		for k = #data.eatenNPCs, 1, -1 do
			...
			if e and e.npc.isValid then
				...
				if (not bypass) and v.ai2 >= cfg.hunger then
					...
					if e.timer == 0 then
						...
						if berries.idMap[e.npc.id] then
							berries.eatFromNPC(e.npc,v);
						else
							e.npc:kill(9);
						end
