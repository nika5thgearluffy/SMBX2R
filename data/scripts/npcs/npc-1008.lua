...
function boomBoom.onTickEndNPC(v)
    ...
    if data.bossStage == 5 then -- Begin Flying: rise to flight height
        data.ai1 = data.ai1 + 1
        ...
        if data.ai1 == 1 then
            v.speedX = 0
            -- Target height: just above the player so they can jump on him
            data.flightTargetY = nearPlayer.y - v.height - 64
            data.boomBoomWalk = true
        end
        
        local distY = data.flightTargetY - v.y
        v.speedY = math.max(-6, distY * 0.15)
        v.speedX = 1.5 * v.direction
        
        if math.abs(distY) < 10 or data.ai1 > 60 then
            data.ai1 = 0
            v.y = data.flightTargetY
            v.speedY = 0
            data.bossStage = 6
        end

        v.animationFrame = boomBoom.flyingFrames[math.max(1, data.animationFramed)]
    end

    if data.bossStage == 6 then -- Flying: patrol toward player
        data.ai1 = data.ai1 + 1
        
        local nearPlayer = Player.getNearest(v.x, v.y)
        
        -- Gradually update flight height to track player vertically
        -- Update target Y slowly so it doesn't snap instantly
        local idealY = nearPlayer.y - v.height - 32
        data.flightTargetY = data.flightTargetY + (idealY - data.flightTargetY) * 0.02
        
        -- Maintain flight height
        local distY = data.flightTargetY - v.y
        v.speedY = distY * 0.1
        
        -- Track player horizontally - move toward their X position
        local playerCenterX = nearPlayer.x + nearPlayer.width / 2
        local boomCenterX = v.x + v.width / 2
        local distX = playerCenterX - boomCenterX
        
        -- Face toward player
        if distX > 70 then
            v.direction = 1
        elseif distX < -70 then
            v.direction = -1
        end
        ...
        if v.x <= sec.boundary.left + 32 then
            v.direction = 1
        elseif v.x + v.width >= sec.boundary.right - 32 then
            v.direction = -1
        end
        
        if data.ai1 > 180 then
            data.ai1 = 0
            data.bossStage = 7
        end
    ...
    if data.bossStage == 7 then -- Dip toward player
        data.ai1 = data.ai1 + 1
        
        local nearPlayer = Player.getNearest(v.x, v.y)
        
        -- On first frame of dip, target the player's current position
        if data.ai1 == 1 then
            data.dipTargetY = nearPlayer.y - v.height  -- dip to player level
        end
        
        v.speedX = 4.5 * v.direction
        
        -- Bounce off walls during dip
        local sec = Section(v.section)
        if v.x <= sec.boundary.left + 32 then
            v.direction = 1
        elseif v.x + v.width >= sec.boundary.right - 32 then
        ...
        -- Sine curve dip toward player level then back up
        local dipProgress = data.ai1 / 60.0
        
        if dipProgress <= 1.0 then
            local dipAmount = math.sin(dipProgress * math.pi)
            local totalDip = data.dipTargetY - data.flightTargetY
            v.y = data.flightTargetY + dipAmount * totalDip
            v.speedY = 0
        else
            v.y = data.flightTargetY
            v.speedY = 0
