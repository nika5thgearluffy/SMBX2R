...
npcManager.setNpcSettings(tarantulaSettings)
...
npcManager.registerHarmTypes(npcID,
	...
	{
		[HARM_TYPE_JUMP]=329,
		[HARM_TYPE_FROMBELOW]=329,
		[HARM_TYPE_NPC]=329,
		[HARM_TYPE_PROJECTILE_USED]=329,
		[HARM_TYPE_LAVA]={id=13, xoffset=0.5, xoffsetBack = 0, yoffset=1, yoffsetBack = 1.5},
		[HARM_TYPE_HELD]=329,
		[HARM_TYPE_TAIL]=10,
		[HARM_TYPE_SPINJUMP]=10,
		[HARM_TYPE_OFFSCREEN] = {id=329, speedY=-2.5},
		[HARM_TYPE_SWORD]=10,
	}
);
...
--Register events
function tarantula.onInitAPI()
	npcManager.registerEvent(npcID, tarantula, "onTickEndNPC")
	registerEvent(tarantula, "onNPCHarm")
end

function tarantula.onTickEndNPC(v)
	...
	if data.state == STATE_INACTIVE then
		if v.collidesBlockBottom then
			...
			if data.timer >= cfg.waitTime then
				v.speedY = -cfg.jumpHeight + RNG.randomInt(-2,2)
				SFX.play(24)
...
function tarantula.onNPCHarm(eventObj,v,reason,culprit)
	local data = v.data
	if v.id ~= npcID then return end
	
	if not data.health then
		data.health = tarantulaSettings.health
	end
